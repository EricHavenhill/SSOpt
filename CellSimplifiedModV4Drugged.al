%----------------------------------------------------------------------
%    File: CellSimplifiedModV4Drugged.al
% Problem: Equations of Motion for the migrating cell
%          run CellSimplifiedModV4Drugged.al
%----------------------------------------------------------------------
...
%jac=[dot(V_Co_N>,N1>);dot(V_Co_N>,N2>)]
%J=coef(jac,[u6,u7])
%Jdot=dt(J)
%Jdotqdot=Jdot*[u6;u7]
% ----------------------------------------------------------
...
% 3rd-Order DMD
x8d = 7E-6 + (1E-8)*(-0.139666289694561)*T + (1/2)*(1E-8)*(0.000119586902340)*T^2 + (1/3)*(1E-8)*(-0.000000011216122)*T^3 + (1/4)*(1E-8)*(0.000000000000276)*T^4
v8d = (1E-8)*(-0.139666289694561) + (1E-8)*(0.000119586902340)*T + (1E-8)*(-0.000000011216122)*T^2 + (1E-8)*(0.000000000000276)*T^3
a8d = (1E-8)*(0.000119586902340) + (2)*(1E-8)*(-0.000000011216122)*T + (3)*(1E-8)*(0.000000000000276)*T^2
...
Input tInitial=0, tFinal=23406.442
...
Input MA = 0.5*6.974E-23 kg, MB = 0.5*6.974E-23 kg
Input MC = 0.5*6.974E-23 kg, MD = 0.5*6.974E-23 kg
Input ME = 0.5*6.974E-23 kg, MF = 0.5*6.974E-23 kg
Input MG = 4E-9 kg

Input grav = 9.81 m/sec^2

Input IA3 = (1/12)*(0.5*6.974E-23)*(20E-6)^2 kg*m^2
Input IB3 = (1/12)*(0.5*6.974E-23)*(20E-6)^2 kg*m^2
Input IC3 = (1/12)*(0.5*6.974E-23)*(20E-6)^2 kg*m^2
Input ID3 = (1/12)*(0.5*6.974E-23)*(20E-6)^2 kg*m^2
Input IE3 = (1/12)*(0.5*6.974E-23)*(20E-6)^2 kg*m^2
Input IF3 = (1/12)*(0.5*6.974E-23)*(20E-6)^2 kg*m^2
Input IG3 = (2/5)*(4E-9)*((7E-6)^2 +(2.5E-6)^2) kg*m^2
...
% Output to MATLAB
output t, q7,q8,q9,u7,u8,u9,FB,FD,FF,det(J)
output A,b,g,GTran

save CellSimplifiedModV4Drugged.all
code Dynamics() CellSimplifiedModV4Drugged_Autolev_2_MATLAB.m